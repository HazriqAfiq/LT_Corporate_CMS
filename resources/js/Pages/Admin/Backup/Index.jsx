        ...
        <AdminLayout header="Backup">
            ...
            <div className="mb-6 flex items-center gap-3 px-5 py-3.5 bg-[var(--gold)]/10 border border-[var(--gold)]/20 rounded-2xl">
                ...
                <div>
                    ...
                    <p className="text-zinc-400 text-xs mt-0.5">UI ini adalah reka bentuk cadangan. Fungsi sebenar memerlukan konfigurasi backend Laravel.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <button className="bg-[#0c0c0e] border border-[var(--gold)]/20 hover:border-[var(--gold)]/40 rounded-2xl p-6 flex flex-col items-center gap-3 transition-all group">
                    ...
                    <div className="text-center">
                        <p className="text-white font-bold text-sm">Jana Backup Sekarang</p>
                        <p className="text-zinc-500 text-xs mt-0.5">Buat backup database manual</p>
                    </div>
                </button>
                <button className="bg-[#0c0c0e] border border-violet-500/20 hover:border-violet-500/40 rounded-2xl p-6 flex flex-col items-center gap-3 transition-all group">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-zinc-500 text-xs mt-0.5">Pulihkan dari backup sedia ada</p>
                    </div>
                </button>
                <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-6 flex flex-col gap-3">
                    <div className="flex items-center gap-2 mb-1">
                        <Clock className="w-4 h-4 text-sky-400" />
                        <p className="text-white font-bold text-sm">Auto Backup</p>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="text-zinc-400 text-xs">Jadual</span>
                        <span className="text-white text-xs font-mono">Setiap hari — 03:00 AM</span>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="text-zinc-400 text-xs">Backup Disimpan</span>
                        <span className="text-white text-xs font-mono">14 hari</span>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className="text-zinc-400 text-xs">Status</span>
                        <span className="inline-flex items-center gap-1 text-emerald-400 text-xs font-semibold">
                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Aktif
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl p-6 mb-6">
                ...
                <p className="text-zinc-600 text-xs mt-2">5.88% daripada had storan</p>
            </div>

            {/* History table */}
            <div className="bg-[#0c0c0e] border border-white/5 rounded-2xl overflow-hidden">
                <div className="px-6 py-4 border-b border-white/5">
                    <h2 className="text-white font-bold text-sm">Sejarah Backup</h2>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-white/5 bg-[#080808]/50 text-zinc-500 text-xs font-semibold uppercase tracking-wider">
                                <th className="px-6 py-3">Nama Fail</th>
                            ...
                            {history.map((b, i) => (
                                <tr key={i} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="px-6 py-3.5">
                                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold border ${b.type === 'Manual' ? 'bg-violet-500/10 text-violet-400 border-violet-500/20' : 'bg-sky-500/10 text-sky-400 border-sky-500/20'}`}>{b.type}</span>
                                    </td>
                                    <td className="px-6 py-3.5 text-xs text-zinc-500 font-mono">{b.date}</td>
                                    <td className="px-6 py-3.5">
                                        {b.status === 'success'
                                            ? <div className="flex items-center gap-1.5 text-emerald-400 text-xs"><CheckCircle2 className="w-3.5 h-3.5" /> Berjaya</div>
                                            : <div className="flex items-center gap-1.5 text-red-400 text-xs"><AlertCircle className="w-3.5 h-3.5" /> Gagal</div>}
                                    </td>
