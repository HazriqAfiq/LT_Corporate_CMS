                                ...
                                return (
                                    <Link key={product.id} href={`/produk/${product.slug}`} className="card group flex flex-col justify-between h-full !rounded-[24px] border border-white/5 hover:border-[var(--gold)]/30 hover:shadow-[0_12px_40px_rgba(234,179,8,0.06)] overflow-hidden transition-all duration-500" data-reveal="fade-up" data-reveal-delay={products.indexOf(product) * 100}>
                                        <div className="flex flex-col">
                                            ...
                                            {/* Floating custom icon wrapper to avoid clipping by overflow-hidden */}
                                            <div className="relative h-0">
                                                <div className="absolute -top-7 left-6 w-14 h-14 rounded-2xl bg-[#0c0c0e] border border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.5)] flex items-center justify-center overflow-hidden z-10 transition-transform duration-500 group-hover:scale-105 group-hover:border-[var(--gold)]/40">
                                                    {product.icon ? (
                                                        <img src={`/storage/${product.icon}`} alt="Icon" className="w-full h-full object-cover" />
                                                    ) : (
                                                        <div className="w-full h-full p-2 bg-gradient-to-br from-[var(--gold)]/20 to-white/5 flex items-center justify-center text-white font-bold text-lg rounded-xl">
                                                            {name.charAt(0)}
                                                        </div>
                                                    )}
                                                </div>
