            ...
            {relatedArticles.length > 0 && (
                <section className="py-24 relative bg-[#080808] border-b border-white/5 z-10 overflow-hidden">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                        <h2 className="section-title text-center mb-12" data-reveal="fade-up">{lang === 'en' ? 'Related ' : 'Artikel '}<span className="gold-accent">{relatedLabel}</span></h2>
                        <div className="grid md:grid-cols-3 gap-8">
                            {relatedArticles.map((a, i) => {
                                const aTitle = (lang === 'en' && a.title_en) ? a.title_en : a.title;
                                return (
                                    <Link key={a.id} href={`/artikel/${a.slug}`} className="card group hover:scale-[1.02] transition-transform duration-300" data-reveal="fade-up" data-reveal-delay={i * 100}>
                                        <div className="p-6">
