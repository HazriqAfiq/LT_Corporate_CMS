    ...
    const fetchMedia = useCallback(async () => {
        ...
        try {
            const response = await axios.get(route('admin.media.index'), {
                params: {
                    search,
                    collection: collection !== 'branding' ? collection : undefined,
                },
                headers: {
        ...
        <Modal show={show} onClose={onClose} maxWidth="4xl">
            <div className="media-picker-modal bg-[#0c0c0e] text-zinc-100 flex flex-col h-[82vh] rounded-2xl overflow-hidden border border-white/5">
                ...
                <div className="media-picker-header px-6 py-5 border-b border-white/5 flex justify-between items-center bg-[#08080a]">
                    ...
                    <button 
                        onClick={onClose} 
                        className="media-picker-close-btn text-zinc-500 hover:text-white transition-colors duration-200 p-1.5 hover:bg-white/5 rounded-lg"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="media-picker-tabs flex border-b border-white/5 px-6 bg-[#08080a]">
                    <button 
                        className={`media-picker-tab-btn py-4 px-5 font-semibold text-sm border-b-2 transition-all duration-300 ${activeTab === 'library' ? 'border-[var(--gold)] text-[var(--gold)]' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
                        onClick={() => setActiveTab('library')}
                    >
                        {t('media_gallery')}
                    </button>
                    <button 
                        className={`media-picker-tab-btn py-4 px-5 font-semibold text-sm border-b-2 transition-all duration-300 ${activeTab === 'upload' ? 'border-[var(--gold)] text-[var(--gold)]' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
                            ...
                            <div className="flex justify-between items-center">
                                <div className="relative w-full max-w-sm">
                                    ...
                                    <TextInput
                                        ...
                                        className="media-picker-search-input pl-10 w-full bg-[#0d0d10] border-white/5 text-white placeholder-zinc-600 focus:border-[var(--gold)]/50 focus:ring-[var(--gold)]/30 rounded-xl"
                                    />
                                </div>
                            </div>
                ...
                <div className="media-picker-footer px-6 py-5 border-t border-white/5 bg-[#08080a] flex justify-between items-center">
                    ...
                    <div className="flex space-x-3">
                        <button 
                            type="button"
                            onClick={onClose} 
                            className="media-picker-btn-cancel px-4 py-2 border text-xs font-semibold uppercase tracking-widest transition-all duration-200 rounded-xl"
                        >
                            {t('cancel')}
                        </button>
                        <PrimaryButton 
                            ...
                            className="media-picker-btn-confirm disabled:opacity-40 disabled:pointer-events-none transition-all duration-300 hover:shadow-[0_0_15px_rgba(234,179,8,0.25)] !rounded-xl"
                        >
                            {t('select_media')}
                        </PrimaryButton>
                    </div>
